...
namespace NeoFPSEditor.Hub.Pages.ItemCreationWizards.PlayerCharacter
{
    class PlayerCharacterStaminaStep : NeoFpsWizardStep
    {
        ...
        public float sprintSpeedMin = 3f;
        [Tooltip(" The highest sprint speed to blend between based on stamina.")]
        public float sprintSpeedMax = 7.5f;
        [Tooltip("The lowest crouch speed to blend between based on stamina.")]
        public float crouchSpeedMin = 1f;
        [Tooltip("The highest crouch speed to blend between based on stamina.")]
        public float crouchSpeedMax = 2f;
        [Tooltip("A multiplier applied to the speed values when the character is aiming down sights.")]
        public float aimingSpeedMultiplier = 0.75f;
        ...
        void OnValidate()
        {
            ...
            // Movement
            walkSpeedMin = Mathf.Clamp(walkSpeedMin, 0.1f, 1000f);
            walkSpeedMax = Mathf.Clamp(walkSpeedMax, 0.1f, 1000f);
            sprintSpeedMin = Mathf.Clamp(sprintSpeedMin, 0.1f, 1000f);
            sprintSpeedMax = Mathf.Clamp(sprintSpeedMax, 0.1f, 1000f);
            crouchSpeedMin = Mathf.Clamp(crouchSpeedMin, 0.1f, 1000f);
            crouchSpeedMax = Mathf.Clamp(crouchSpeedMax, 0.1f, 1000f);
            aimingSpeedMultiplier = Mathf.Clamp01(aimingSpeedMultiplier);
                ...
                case 1: // Full stamina
                    {
                        ...
                        {
                            if (NeoFpsEditorGUI.MotionGraphFloatDataKeyField(serializedObject.FindProperty("walkSpeedData"), motionGraph))
                            {
                                ++EditorGUI.indentLevel;
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("walkSpeedMin"));
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("walkSpeedMax"));
                                --EditorGUI.indentLevel;
                            }
                            if (NeoFpsEditorGUI.MotionGraphFloatDataKeyField(serializedObject.FindProperty("sprintSpeedData"), motionGraph))
                            {
                                ++EditorGUI.indentLevel;
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("sprintSpeedMin"));
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("sprintSpeedMax"));
                                --EditorGUI.indentLevel;
                            }
                            if (NeoFpsEditorGUI.MotionGraphFloatDataKeyField(serializedObject.FindProperty("crouchSpeedData"), motionGraph))
                            {
                                ++EditorGUI.indentLevel;
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("crouchSpeedMin"));
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("crouchSpeedMax"));
                                --EditorGUI.indentLevel;
                            }

                            bool showAimMult = false;
                            showAimMult |= NeoFpsEditorGUI.MotionGraphFloatDataKeyField(serializedObject.FindProperty("aimWalkSpeedData"), motionGraph);
                            showAimMult |= NeoFpsEditorGUI.MotionGraphFloatDataKeyField(serializedObject.FindProperty("aimSprintSpeedData"), motionGraph);
                            showAimMult |= NeoFpsEditorGUI.MotionGraphFloatDataKeyField(serializedObject.FindProperty("aimCrouchSpeedData"), motionGraph);
                            if (showAimMult)
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("aimingSpeedMultiplier"));
                        }

        ...
        void InspectGamepadProfiles()
        {
            ...
            for (int i = 0; i < gamepadProfiles.arraySize; ++i)
            {
                ...
                if (expanded.boolValue)
                {
                    ...
                    var buttons = profile.FindPropertyRelative("m_Buttons");
                    for (int j = 0; j < buttons.arraySize; ++j)
                    {
                        EditorGUILayout.PropertyField(buttons.GetArrayElementAtIndex(j), new GUIContent(((GamepadButton)j).ToString()));
                    }

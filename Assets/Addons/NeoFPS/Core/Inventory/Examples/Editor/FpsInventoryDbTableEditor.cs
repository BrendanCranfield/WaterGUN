...
namespace NeoFPSEditor
{
    ...
    public class FpsInventoryDbTableEditor : Editor
    {
        ...
        void DrawManageDatabaseControls()
        {
            ...
            var other = EditorGUILayout.ObjectField("Import Database", null, typeof(FpsInventoryDbTable), false);
            if (other != null)
            {
                // Get entries property
                var entries = serializedObject.FindProperty("m_Entries");

                // Get other entries property
                var so = new SerializedObject(other);
                var otherEntries = so.FindProperty("m_Entries");

                // Merge
                MergeEntries(entries, otherEntries);

...
namespace NeoFPS
{
	...
	public abstract class FpsInventoryBase : MonoBehaviour, IInventory, IQuickSlots, INeoSerializableComponent
	{
		...
		[SerializeField, Tooltip("The transform to set as the parent of any objects added to the inventory.")]
		...
		[SerializeField, Tooltip("A proxy transform used to set the drop position and rotation when a wieldable item is dropped.")]
		...
		[SerializeField] private FpsInventoryWieldable m_BackupItem = null;
        [Tooltip("A selection of inventory items to be added to the inventory on start.")]
        [SerializeField] private FpsInventoryItemBase[] m_StartingItems = new FpsInventoryItemBase[0];

...
namespace NeoFPS.ModularFirearms
{
	...
	public class StandardShellEject : BaseEjectorBehaviour
    {
        ...
        [SerializeField, RequiredObjectProperty, Tooltip("A proxy transform where ejected shells will be spawned.")]
		private Transform m_ShellEjectProxy = null;

        [SerializeField, RequiredObjectProperty, Tooltip("The shell prefab object to spawn.")]
		...
		void DoEject ()
		{
			if (m_ShellPrefab != null)
			{
                ...
                if (m_InheritVelocity > 0f && firearm.wielder != null)
                    velocity += firearm.wielder.motionController.characterController.velocity * m_InheritVelocity;
                ...
                IBulletCasing casing = PoolManager.GetPooledObject<IBulletCasing> (
                    m_ShellPrefab,
                    m_ShellEjectProxy.position + (velocity * Time.deltaTime),

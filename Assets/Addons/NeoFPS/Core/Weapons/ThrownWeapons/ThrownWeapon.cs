...
namespace NeoFPS
{
    ...
    public class ThrownWeapon : MonoBehaviour, IThrownWeapon, IWieldable, IDamageSource, ICrosshairDriver, IPoseHandler, INeoSerializableComponent
    {
		[SerializeField, Tooltip("A proxy transform for setting the position and rotation of the spawned projectile (weak throw).")]
		private Transform m_ProjectileSpawnPointWeak = null;

        [SerializeField, Tooltip("A proxy transform for setting the position and rotation of the spawned projectile (strong throw).")]
		private Transform m_ProjectileSpawnPointStrong = null;

        [SerializeField, Tooltip("The prefab to throw.")]
		private PooledObject m_SpawnedProjectile = null;

        [SerializeField, Tooltip("The weapon game object. This is deactivated and swapped with the pooled object during the throw animation.")]
        ...
        {
            if (m_InventoryItem != null && m_InventoryItem.owner == null)
                Destroy(gameObject);
        }
        ...
        void OnEnable ()
        {
            ...
            if (m_AudioSelect != null)
                m_AudioSource.PlayOneShot(m_AudioSelect, FpsCharacterAudioSource.Head);
        }
		...
		IEnumerator ThrowCoroutine (float duration, float delay, float speed, Transform spawnPoint)
		{
			...
			if (m_InheritVelocity > 0f && m_InventoryItem != null)
			{
				ICharacter c = m_InventoryItem.owner;
				if (c != null)
				{
                    Vector3 inherited = c.motionController.characterController.velocity;
					//inherited.y = 0f;
